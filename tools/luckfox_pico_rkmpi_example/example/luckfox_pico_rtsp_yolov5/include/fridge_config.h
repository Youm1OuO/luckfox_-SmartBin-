...
// -------------------------------------------------------------------------
//  判断"消失的物品是不是被别的物品挡住了"。
//  coverage_ratio(A,B) = area(A ∩ B) / area(A) > 0.6  → A 被 B 覆盖了大部分
//  或者 center_distance(A,B) < 40  → B 的中心在 A 附近（大小差异大的兜底）
// =========================================================================
constexpr float OCCLUDE_OVERLAP_THRESH = 0.6f;       // coverage_ratio 阈值
constexpr float OCCLUDE_CENTER_DIST_THRESH = 40.0f;  // 遮挡中心距离阈值（像素）

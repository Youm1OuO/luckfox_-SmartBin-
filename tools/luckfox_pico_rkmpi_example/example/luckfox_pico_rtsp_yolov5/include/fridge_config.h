...
//  5个条件全部满足才算同一个物品：类别 + 中心距离 + IoU + 面积比 + 颜色差异。
// =========================================================================
constexpr float IDENTITY_CENTER_DIST = 15.0f;   // 中心点距离阈值（像素）
constexpr float IDENTITY_IOU_THRESH  = 0.7f;    // IoU 阈值
constexpr float IDENTITY_AREA_RATIO  = 0.6f;    // 面积比阈值（较小/较大，>=此值才算大小接近）
constexpr float IDENTITY_COLOR_DIFF  = 20.0f;   // 平均 RGB 差异阈值（每通道 0~255）

// =========================================================================
//  新业务流程4-1：身份匹配阈值（宽松）
// -------------------------------------------------------------------------
//  用于N帧计数中的帧间匹配、方法B的快照对比。
//  只需类别相同 + 位置大致相同（基于bbox对角线的比例）。
// =========================================================================
constexpr float IDENTITY_LOOSE_DIAG_RATIO = 0.3f;  // 距离阈值 = bbox对角线 * 此比例

// =========================================================================
//  新业务流程4-1：附近检测阈值
// -------------------------------------------------------------------------
//  第一步中判断"新物品C是否在消失物品A的原位置附近"。
//  使用 overlap_ratio_of_smaller(A.box, C.box) > 此值。
// =========================================================================
constexpr float NEARBY_OVERLAP_THRESH = 0.15f;

// =========================================================================
//  新业务流程4-1：N帧快照参数
// -------------------------------------------------------------------------
//  用N帧的综合结果作为一个微快照，避免YOLO抖动误判。
//  N必须为奇数，物品必须出现超过 N*THRESHOLD 帧才算识别到。
// =========================================================================
constexpr int   N_SNAPSHOT_FRAMES    = 3;      // 每N帧做一个微快照
constexpr float N_SNAPSHOT_THRESHOLD = 0.6f;   // 出现超过60%的帧才算识别到
constexpr int   MIN_SEGMENT_SNAPSHOTS = 3;     // 一个时间段至少要维持的微快照数（防止过度分割）

// =========================================================================
//  新业务流程4-1：遮挡检测阈值
// -------------------------------------------------------------------------
//  判断"消失的物品是不是被新物品压住了"。只需要空间重叠，不需要同类同色。
// =========================================================================

...
constexpr const char* FLOW3_BUILD_TAG = "3.0-r22-color";
...
// 演示专用颜色二次判别开关
// -------------------------------------------------------------------------
// true: 仅在 apple/onion、egg/orange 上使用中心 ROI 的 HSV/面积强证据
//       修正 cls_id；证据不足时仍保持 YOLO 原结果。
// false: 完全跳过该规则，恢复原有 YOLO 后处理行为。
// =========================================================================
constexpr bool ENABLE_COLOR_POSTPROCESS = true;

// 需要查看真实框尺寸和 HSV 数值时可临时改为 true；默认关闭，避免大量
// 终端输出影响演示时的实时性。
constexpr bool ENABLE_COLOR_POSTPROCESS_DEBUG_LOG = false;

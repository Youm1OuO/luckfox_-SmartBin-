...
> 直到这一步, 我们可以大胆认为:
1. 库存中剩下的没匹配上的物品A (.affirm == False) 
    > 1.1 如果原先是【可见】的话, 要么是[被完全遮挡了]要么是[被整理到其他地方]或者[被拿走出库了]
    > 1.2如果原先是【遮挡】的话, 要么是[依旧被完全遮挡]或者[其实已经露出了一点了,但是还没匹配上]
...
for B in 快照物品 :
    if B.item_id == -1:
        for A in 库存物品 :
            if !A.affirm and (A.cls_id==B.cls_id) and (abs(B.box.width - A.box.width) <= eps_w) and (abs(B.box.height - A.box.height) <= eps_h):
